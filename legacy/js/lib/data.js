 ...
 *
 * Both JSON files are bundled at build time, so the whole site works as
...
export { categories, products, branches };
...
export const getCategory = (slug) => byCategorySlug.get(slug) ?? null;
export const getSubcategory = (slug) => subIndex.get(slug) ?? null;
export const getProduct = (sku) => bySku.get(sku) ?? null;

export const productsInCategory = (slug) => products.filter((p) => p.category === slug);
export const productsInSubcategory = (slug) => products.filter((p) => p.subcategory === slug);

export const countInCategory = (slug) => productsInCategory(slug).length;
export const countInSubcategory = (slug) => productsInSubcategory(slug).length;

export const allBrands = [...new Set(products.map((p) => p.brand))].sort((a, b) =>
  a.localeCompare(b),
);

export const brandsInCategory = (slug) =>
  [...new Set(productsInCategory(slug).map((p) => p.brand))].sort((a, b) => a.localeCompare(b));

/** Everything tagged `popular`, in catalogue order. */
export const popularProducts = (limit = 10) =>
 ...
 * typing "tapa" surfaces Tapal before anything that merely contains
...
export function searchAll(rawQuery, limit = 12) {
  ...
  for (const product of products) {
    ...
    const best = [name, brand].filter(Boolean).sort((a, b) => b.score - a.score)[0];
    if (!best) continue;
  ...
  for (const cat of categories) {
    ...
    if (hit) results.push({ kind: 'category', id: cat.slug, record: cat, score: hit.score + 20, match: hit });
...
export function refineProducts(list, { brand = '', sort = 'featured' } = {}) {
  let out = brand ? list.filter((p) => p.brand === brand) : [...list];
